        ...
        if(child.getButton()){
        	...
        	if(child.getText().equals("Reject")){
        		...
        		( convertView.findViewById(R.id.optionalChildButton)).setOnClickListener(new OnClickListener(){
					...
					public void onClick(View arg0) {
						// TODO Auto-generated method stub
						String query = "delete from Recommendations where RecommendationID = " + MainForm.getRecommendationsList().get(groupPosition-MainForm.getMyList().getList().size()).getRecID(); //delete just that one
